...
export function LimitInfo() {
  ...
  const changeListRateValue = (processIndex: number,rateIndex: number,value: string, type: ("rate"|"value"|"path") ) => {
    const list = (selectProcess?.reProcessdata ?? []).map((item,index) => {
      if(processIndex === index) {
        ...
        <div className="process-select-view">
          <div className="select-info flex-nw">
            <div className="select-info-img maxhalf50 pr-1">
              目的の画像
              {selectProcess.mainImage && <img className="img" src={selectProcess.mainImage} alt="" />}
            </div>
            <div className="select-info-text maxhalf50">
              <div className="item pb-1 f-middle">
                {selectProcess?.title}
              </div>
              <div className="item pb-1">
                {selectProcess?.detail}
              </div>
            </div>
          </div>
          <div className="item pb-1">
          ...
          <div className="item pb-1 flex-nw over-scroll">
            <div className="minh150 flex-nw p-1" style={{ minWidth: `${(selectProcess?.reProcessdata ?? []).length * 320}px` }}>
              {(selectProcess?.reProcessdata ?? []).map((item,index) => <div
                  ...
                  <div className="fields p-1">
                    <div className="field">
                      <div
                        ...
                        onClick={()=> addListRateValue(index)}
                      >add</div>
                    </div>
                    <div className="field">
                      <div className="view-rate flex-nw">
                        {(item?.ratedata ?? []).map((item,rateIndex) => <div key={`ratedata01${rateIndex}`} className={`item${rateIndex} p-1`} style={{width: `${Number(item.rate)*100}%`}}>{item.rate}</div>)}
                      </div>
                    </div>
                  </div>
                  {(item?.ratedata ?? []).map((item,rateIndex) => <div key={`ratedata02${rateIndex}`} className="">
                    <div className="fields p-1">
      ...
      <div className="fields p-2">
        <div className="field pb-1">
          ...
          <BaseDialog ref={element} phase="20">
            <div className="fields p-2">
              ...
              {rateList.map((item,index) => <div key={`rateList${item.id}`} className="">
                <FieldInput
                  id={`text${index}`}
                  label=""
                  value={item.value}
        ...
        <div className="field pb-1">
          <div className="premise-info">
            ...
            <div className="pb-2">
              <div className="select-info pb-1">kokokoko  [https://www.takasago-t.co.jp/index.php]</div>
              <div className="select-tag mr-1"
                onClick={() => {}}
              >電気ヒータ</div>
            ...
            <div className="pb-1">
              <div className="select-info pb-1">田中貴金属工業株式会社  [https://tanaka-preciousmetals.com/webexpo/metal/]</div>
              <div className="select-tag mr-1"
                onClick={() => {}}
              >ゴールド（金）</div>

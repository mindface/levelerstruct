...
export function ProcessCards(props: Props) {
  ...
  return (
    <div>
      <div className="select-method over-scroll flex-nw scroll-shadow">
        <div className="minh150 flex-nw p-1" style={{ minWidth: `${makeProcess.length * 320}px` }}>
          {makeProcess.map((item, index) => (
            <div
              key={`${item.methodId}-${index}`}
              className="method-card max320 box-shadow p-1 positionbase"
            >
              {type === "edit" && (
                <span className="close cursor box-shadow" onClick={() => deleteChangeAction(index)}>
              ...
              <div className="fields">
                <div className="field pb-1">
                  手法タイトル : {item.title}
                  {type !== "connection" && (
                    <button className="btn f-small" onClick={() => movePage(item.methodId)}>
                      move
                    </button>
                  )}
                </div>
                {type !== "connection" && (
                  <div className="field pb-1">
                    <div className="similar-method positionbase cursol-absolute-view p-1">
                      <span
                        className="background-sub-color d-inline p-1 f-small"
                        onClick={() => brnachAction(item)}
                      >
                        比較プロット作成
                      </span>
                          ...
                          {brnachAction(item).map((brnachItem, k) => {
                            return (
                              <li
                                key={`brnachItem${k}`}
                                className="p-1"
                                onClick={() => alliesMethod(brnachItem, index)}
                              >
                                <div className="">{brnachItem.title}</div>
                              </li>
                            );
                          })}
                        </ul>
                ...
                <div className="field pb-1">
                  調整範囲 - <br />
                  {type === "view" && <div className="detail">{item.detail}</div>}
                  {type === "edit" && (
                    <textarea
                      className="textarea"
                      cols={10}
                ...
                <div className="field pb-1">
                  {(item.adjustmentNumbers as number[])?.map(
                    (adjustmentNumber: number, index: number) => {
                      return (
                        <span key={`adjustmentNumber${index}`} className="d-b p-1">
                          {adjustmentNumber + (rate ?? 0)}
                        </span>
                      );
                    }
                  )}
      ...
      {makebrnachs.map((brnachItem) => (
        <div key={brnachItem.id} className="select-method over-scroll flex-nw scroll-shadow">
          <div

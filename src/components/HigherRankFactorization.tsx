...
export function HigherRankFactorization() {
  ...
  return (
    <div className={viewSwitch ? "factorization struct" : "factorization"}>
      <h3 className="title p-1">
        エリア情報での言語情報を手段規定
        <span className="btn" onClick={viewSwitchAction}>
          {viewSwitch ? "構造化" : "調整"}
        </span>
      </h3>
      <div className="flex p-1">
        {conceptualizations.map((item, index) => (
          ...
          >
            ...
            <p className="pb-1">
              {" "}
              (実行数){item.runNumber} | (達成数){item.purposeAchieved}
            </p>
      ...
      <div className={viewSwitch ? "planning-settings" : "planning-settings flex"}>
        ...
        {viewSwitch && (
          <>
            ここにドロップ{" "}
            <button className="btn" onClick={resetConceptualizationAction}>
              リセット
            </button>
          </>
        )}
        <div className="draggable-area">
          ...
          >
            {conceptualizationsPlan.map((item: ClusterTask, index: number) => (
              <div key={`${item.id}-${index}`} className={viewSwitch ? "" : "flex"}>
                ...
                {!viewSwitch && (
                  <div className="p-1">
                    <p>プロセス情報</p>
                    <BaseDialog ref={element} phase="20" openBtnView={true}>
                      <ProcessCards
                        type="view"
                        processItems={item?.processItem[0]?.processdata ?? []}
                      />
                    </BaseDialog>
                  </div>
                )}

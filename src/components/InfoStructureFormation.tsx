...
export function InfoStructureFormation() {
  ...
  return (
    <div className="content">
      ...
      <div className="select-process">
        <div className="process-box cursol-absolute-view positionbase d-inline max420 p-1">
          ...
          <ul className="list hover-absolute-view maxh320 over-scroll background-white box-shadow">
            {process.map((item) => <li key={item.id} className="item divhover cursor p-1" onClick={() => addProcess(item)}>
              {item.title}
            </li>)}
          </ul>
        </div>
      ...
      <div
        className="structure-area positionbase minh320"
        ref={structureArea}
      >
        { bgUrl !== "" && <img className="img" src={bgUrl} /> }
        {makeStructureComparisons.map((item) => <DraggableDiv
          type="grab"
          containerRect={{
        ...
        >
        <div className="cursol-absolute-view positionbase">{item.title}
